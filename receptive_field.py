...
def receptive_field(model, input_size, batch_size=-1, device="cuda"):
    ...
    def register_hook(module):
        ...
        implemented_rf_variant = ['Conv2d', 'MaxPool2d', 'CoConvZ2', 'CoConvCo', 'GroupMaxPool2d', 'P4ConvZ2', 'P4ConvP4']
        implemented_rf_invariant = ['BatchNorm2d', 'BatchNorm3d', 'ReLU', 'Bottleneck', 'Dropout2d', 'GroupCosetMaxPool']
        ...
        def hook(module, input, output):
            ...
            if not receptive_field["0"]["conv_stage"]:
                ...
                if class_name in implemented_rf_variant: # Layers that do change rf
                ...
                elif class_name in implemented_rf_invariant: # Layers that do not change rf
                    ...
                    raise NotImplementedError("Module {} not implemented.".format(class_name))
                    pass
    ...
    print("------------------------------------------------------------------------------")
    line_new = "{:>20}  {:>10} {:>12} {:>10} {:>20} ".format("Layer (type)", "map size", "start", "jump", "receptive_field")
    print(line_new)
    print("==============================================================================")
    total_params = 0
    total_output = 0
...
def receptive_field_for_unit(receptive_field_dict, layer, unit_position, bound=True):
    ...
    if layer in receptive_field_dict:
        ...
        if bound:
            rf_range = [(max(0, int(rf_range[axis][0])), min(int(limit[axis]), int(rf_range[axis][1]))) for axis in range(2)]
        else:
            rf_range = [(int(rf_range[axis][0]), int(rf_range[axis][1])) for axis in range(2)]
